...
const ToastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border-2 p-6 pr-8 shadow-lg transition-all ' +
    ...
    'data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full',
  {
    variants: {
      variant: {
        default:
          'border-black dark:border-white bg-background dark:bg-dark-background text-foreground dark:text-dark-foreground',
        destructive:
          'destructive group border-black dark:border-white bg-red-600 text-white',
      },
    },
    defaultVariants: {
      variant: 'default',
...
const ToastClose = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 opacity-100",
